#!/usr/bin/env python3
"""
Step-by-step fix for the raymarching shader
Start with simple geometry and add complexity gradually
"""

import os
import json

def create_progressive_shader_fix():
    """Create a JavaScript file that progressively fixes the shader"""
...
''';
    ...
    output_path = os.path.join(os.path.dirname(__file__), '..', 'FIRM_ui', 'progressive_shader_fix.js')
    
    with open(output_path, 'w') as f:
        f.write(fix_js)
    
    print(f"✅ Created progressive shader fix at {output_path}")
    return output_path

def add_to_index():
    """Add the progressive fix to index.html"""
    ...
    if 'progressive_shader_fix.js' in content:
        print("⚠️ Progressive shader fix already in index.html")
        return False
    
    # Add before </head>
    insertion_point = content.find('</head>')
    if insertion_point > 0:
        script_tag = '    <script src="progressive_shader_fix.js"></script>\n'
        content = content[:insertion_point] + script_tag + content[insertion_point:]
        
        with open(index_path, 'w') as f:
            f.write(content)
        
        print("✅ Added progressive shader fix to index.html")
        return True
    
    return False

def main():
    print("🔧 Creating Progressive Shader Fix...")
    print("=" * 50)
    
    # Create the progressive fix
    shader_path = create_progressive_shader_fix()
    
    # Add to HTML
    add_to_index()
    
    print("=" * 50)
    print("✅ Progressive shader fix ready")
    print("\n📝 Instructions:")
    print("1. Reload the page")
    print("2. Open console")
    print("3. Run: window.progressiveShaderFix.test()")
    print("4. Start with: window.progressiveShaderFix.applyLevel(0)")
    print("5. Increase complexity: window.progressiveShaderFix.nextLevel()")
    print("6. Visualize: window.visualizeCustomShader()")
    print("\nThis will help identify exactly where the shader breaks.")
